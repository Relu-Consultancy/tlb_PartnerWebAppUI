...
export const CreateProgramPolicies: React.FC<Props> = ({ onNavigate }) => {
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
        <header className="bg-white p-6 flex items-center justify-between sticky top-0 z-30 border-b border-gray-100">
            <button onClick={() => onNavigate('CREATE_PROGRAM_MEDIA')} className="p-2 -ml-2"><ArrowLeft size={24} /></button>
            <div className="text-center">
                <h1 className="font-black text-lg">New Listing</h1>
                <p className="text-[10px] font-bold text-tlb-yellow uppercase tracking-widest">Stage 4 of 5 â€” Entry Path & Policies</p>
            </div>
            <div className="w-10" />
        </header>
        ...
        <main className="p-6">
            <div className="tlb-content space-y-6">
                ...
                <div className="flex gap-3">
                    <button onClick={() => onNavigate('CREATE_PROGRAM_MEDIA')} className="flex-1 py-4 border border-gray-200 rounded-2xl font-bold text-gray-500 text-sm">
                        â† Back
                    </button>
                    <button onClick={() => onNavigate('CREATE_PROGRAM_PREVIEW')} className="flex-1 tlb-button py-4 gap-2">
